    ...
    for label in st.session_state.button_labels:
        if st.button(label):
            with st.spinner("Thinking..."):
                ...
                st.session_state.messages.append({"role": "user", "content": f"Please design this converter for me. List things such as switching frequency, DCM vs. CCM, L and C values, turn ratios, etc.; Keep it short and only list results. could you now build a table that has switching frequency, duty cycle and turns ratio as parameters, and show the related other parameters (output/input current, magnetizing inductance, and other if applicable). Only show feasable designs"})
